...
; install the current locale at startup, via a call to (setlocale LC_ALL
; "")1 . See Section 7.2.13 [Locales], page 554, for more information on
; locales.  1 The

; GUILE_INSTALL_LOCALE

; environment   variable  was  ignored   in  Guile
; versions prior to 2.0.9.


; Guile Reference Manual

; You may explicitly indicate that you do not want to install the locale
; by  setting

; GUILE_INSTALL_LOCALE

; to 0, or explicitly enable it  by setting the variable to 1.  Usually,
; installing the  current locale  is the  right thing  to do.  It allows
; Guile to correctly parse and  print strings with non-ASCII characters.
; Therefore, this option is on by default.

; GUILE_LOAD_COMPILED_PATH

; This variable  may be used  to augment the  path that is  searched for
; compiled Scheme files (‘.go’ files)  when loading. Its value should be
; a colon-separated list of directories. If it contains the special path
; component ... (ellipsis), then the default path is put in place of the
; ellipsis, otherwise the default path is placed at the end.  The result
; is  stored in  %load-compiled-path (see  Section 6.16.8  [Load Paths],
; Here is  an example  using the Bash  shell that  adds the
; current directory, ‘.’, and  the relative directory ‘../my-library’ to
; %load-compiled-path:
...
; in %load-path (see Section 6.16.8 [Load Paths], page 393).  Here is an
; example using  the Bash shell  that prepends the current  directory to
; %load-path, and adds the relative directory ‘../srfi’ to the end:
...
; colon-separated  (semicolon  on  Windows)  list  of  directories.  See
; Section 6.19.1 [Foreign Libraries].
...
; occurs,  Guile will  issue warn-  ing  messages about  code that  uses
; features that have been marked for eventual
